...
<div class="card-body">
    ...
    <div class="mt-3">
        <h6>Mau Pesan Apa Hari ini?</h6>
        <hr>
        <div class="d-flex gap-2 align-items-center flex-wrap mb-3">
            <a href="{{ route('home') }}" class="btn btn-outline-danger {{ request()->query('category_id') ? '' : 'active' }}">
                All
            </a>
            @foreach ($categories as $category)
                <a href="{{ route('home', ['category_id' => $category->id]) }}" class="btn btn-outline-danger {{ request()->query('category_id') == $category->id ? 'active' : '' }}">
                    {{ $category->name }}
                </a>  
            @endforeach
          </div>
                ...
                @forelse ($products as $product)
                    <div class="card" style="width: 30%">
                        ...
                        <div class="card-body">
                            ...
                            <hr>
                            <h5>Merchant : {{ $product->user->name }}</h5>
